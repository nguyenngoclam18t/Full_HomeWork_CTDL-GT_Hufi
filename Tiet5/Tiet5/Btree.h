...
int Remove_X(NODET<T>*& root, T X, int (*Compare)(T, T)) {
	if (!root)return 0;
	if (Compare(root->Data, X) > 0) Remove_X(root->Left, X,Compare);
	else if (Compare(root->Data, X) < 0) Remove_X(root->Right, X, Compare);
	else
	{
...
void traverseRNL(NODET<T>* root, void(*show)(NODET<T>*)) {
	if (root == NULL)return;
	traverseRNL(root->Right, show);
	show(root);
	traverseRNL(root->Left, show);
}
template<class T>
